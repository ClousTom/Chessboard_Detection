...
widthWarpedImage = 800
heightWarpedImage = 800
...
############
# #Find only vertical and horizontal lines with Probabilistic Hough T.
# def VerHorLinesP(lines):
#     dif=5
#     outL = []
#     for line in range(len(lines)):
#         if ((lines[line][0][0]-lines[line][0][2]<=dif and lines[line][0][0]-lines[line][0][2]>=-dif) or (lines[line][0][1]-lines[line][0][3]<=dif and lines[line][0][1]-lines[line][0][3]>=-dif)):
#             outL.append(lines[line])
#     return outL
...
#     l = len(list)
#     for i in range(l):
#         if all(dist(list[i], p) > 20 for p in out):
#             out.append(list[i])
#     return out
...
#         else:
#             cv2.line(warped_img, (x1,y1), (x2,y2), (255,100,50), 1)
#     pts = []
...
def CentralPoint(square):
    x=((square[1][0]-square[0][0])/2)+square[0][0]
    y=((square[2][1]-square[0][1])/2)+square[0][1]
    point=[x,y]
    return point

# def FindInside(image,squares):
#     for x in range(len(squares)):
#         point = CentralPoint(squares[x])
...
# #Detect and delete Green pixels
...
############
# #Probabilistic Hough T.
# lines = cv2.HoughLinesP(canny_img, 1, np.pi/180, 60, minLineLength=40, maxLineGap=70)
# lines = VerHorLinesP(lines)
...
# for pt in pts:
#     # print(pt)
#     cv2.circle(warped_img, (int(pt[1]), int(pt[0])), 3, (0,255,0), 1)

# for line in lines:
#     # x1, y1, x2, y2, l, p, n = line
#     x1, y1, x2, y2 = line[0]
#     # print(line)
#     cv2.line(warped_img, (x1,y1), (x2,y2), (0,0,255), 1)
##############
...
if lines is not None:
    ...
    # FindInside(warped_img2,squares)
